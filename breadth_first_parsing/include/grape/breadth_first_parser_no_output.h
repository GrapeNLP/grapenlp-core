...
#ifndef GRAPE_BREADTH_FIRST_PARSER_NO_OUTPUT_H
#define GRAPE_BREADTH_FIRST_PARSER_NO_OUTPUT_H
...
namespace grape
{
	...
	struct breadth_first_parser_no_output
	{
		...
		struct acceptor_execution_state_acknowledger
		{
			...
			inline void operator()(const execution_state &x)
			{
				...
				if (x.q->is_final() && x.pi.empty())
					std::wcout << "acceptor execution state found" << std::endl;
#endif
			}
		};
		...
		template<typename ExtraInsertOp>
		inline void build_initial_ses(state_const_ref initial_state, state_const_ref_stack empty_stack, bool hasnt_white_at_begin, ExtraInsertOp op)
		{
#ifdef TRACE
		...
		bool operator()(const machine& grammar, SourceRef input_begin, SourceRef input_end, bool hasnt_white_at_begin, bool hasnt_white_at_end)
		{
			...
			if (input_begin == input_end)
			{
				...
				eclosure(input_begin, hasnt_white_at_begin, ack_op);
			}
			...
			else
			{
				...
				if (!v.empty())
				{
					...
					eclosure(input_begin_plus_1, hasnt_white_at_end, ack_op);
					++input_begin;
				}
			}
#ifdef TRACE
...
#endif /*GRAPE_BREADTH_FIRST_PARSER_NO_OUTPUT_H*/
