...
void help(const grape::string &program_name, const options_description &desc)
{
	...
	if (is_corpus) //Input is a corpus of sentences
	{
		...
		if (compute_fprtn_stats)
		{
			...
			for (; sri != srv.end(); ++sri, ++rri) //for each sentence
			{
				...
				u_fputc(output_bound_char, f);
				u_fprintf(f, "\n\n");
				if (validate && !grape::equal(rri->left, rri->right, the_manager.get_output_u_array().begin(), the_manager.get_output_u_array().end())) //if unexpected result, write also the expected one
				{
