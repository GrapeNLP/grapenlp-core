...
{
	FILE *bin_delaf_file(fopen("/opt/Unitex/English/Dela/dela-en-public.bin", U_READ));
	if (!bin_delaf_file)
		fatal_error("Unable to open bin delaf file\n");
	FILE *inf_delaf_file(u_fopen("/opt/Unitex/English/Dela/dela-en-public.inf", U_READ));
	if (!inf_delaf_file)
...
{
	FILE *f(u_fopen("../dico-en.txt", U_WRITE));
	if (f == NULL)
