...
#ifndef GRAPE_LRB_TREE_3W_H
#define GRAPE_LRB_TREE_3W_H

#include <iostream>
#include <stack>
#include <grape/functional.h>
#include <grape/memory.h>
...
*/
	...
	private:
		inline std::ptrdiff_t compare_value_keys_3w(const value_type &v1, const value_type &v2) const
		{ return d.comp(KeyGetter()(v1), KeyGetter()(v2)); }

		inline std::ptrdiff_t compare_value_keys_3w(const value_type &v, base_node_const_ref x) const
		{ return d.comp(KeyGetter()(v), KeyGetter()(x->value())); }

		inline std::ptrdiff_t compare_value_keys_3w(base_node_const_ref x, const value_type &v) const
		{ return d.comp(KeyGetter()(x->value()), KeyGetter()(v)); }

		inline std::ptrdiff_t compare_keys_3w(const key_type &k1, const key_type &k2) const
		{ return d.comp(k1, k2); }

		inline std::ptrdiff_t compare_keys_3w(base_node_const_ref x, const key_type &k) const
		{ return d.comp(KeyGetter()(x->value()), k); }

		inline std::ptrdiff_t compare_keys_3w(const key_type &k, base_node_const_ref x) const
...
*/
	...
	private:
		...
		inline base_node_ref insert_with_default_mapped_value_as_root(const key_type& k)
		...
		inline base_node_ref insert_with_default_mapped_value_as_left_child_of(base_node_ref x, const key_type& k)
		...
		inline base_node_ref insert_with_default_mapped_value_as_right_child_of(base_node_ref x, const key_type& k)
	...
	public:
		...
		//Insertion without duplicates (uses "less than" comparator)
		inline std::pair<iterator, bool> insert_with_default_mapped_value(const key_type &key)
		{
		...
		//Insertion with duplicates (uses "greater or equal than" comparator)
		inline iterator multi_insert_with_default_mapped_value(const key_type &key)
	...
	public:
		...
		inline std::pair<iterator, bool> insert_with_default_mapped_value(const_iterator position, const key_type& k)
		{
		...
		inline iterator multi_insert_with_default_mapped_value(const_iterator position, const key_type& k)
...
//				z->key() = y->key();
	...
	public:
		...
		inline bool operator== (const lrb_tree_3w& t) const
	...
	inline void swap(lrb_tree_3w<Key, Value, KeyGetter, KeyCompare3Way>& x, lrb_tree_3w<Key, Value, KeyGetter, KeyCompare3Way>& y)
	{ x.swap(y); }

	template<typename Key, typename Value, typename KeyGetter, typename KeyCompare, typename Allocator>
	inline typename lrb_tree_3w<Key, Value, KeyGetter, KeyCompare, Allocator>::value_type& lrb_tree_3w<Key, Value, KeyGetter, KeyCompare, Allocator>::base_node::value()
	{ return static_cast<node_ref>(this)->v; }

	template<typename Key, typename Value, typename KeyGetter, typename KeyCompare, typename Allocator>
	inline const typename lrb_tree_3w<Key, Value, KeyGetter, KeyCompare, Allocator>::value_type& lrb_tree_3w<Key, Value, KeyGetter, KeyCompare, Allocator>::base_node::value() const
	{ return static_cast<node_const_ref>(this)->v; }
} //namespace grape

#endif /*GRAPE_LRB_TREE_3W_H*/
