...
#ifndef GRAPE_LRB_TREE_SET_3W_H
#define GRAPE_LRB_TREE_SET_3W_H
...
*/
    ...
    public:
		...
		lrb_tree_set_3w(): the_tree()
		{}

		explicit lrb_tree_set_3w(const Compare& comp_, const allocator_type& alloc_ = allocator_type()): the_tree(comp_, alloc_)
		{}

		template<typename Iterator>
		lrb_tree_set_3w(Iterator first, Iterator last): the_tree()
		{ the_tree.insert(first, last); }

		template<typename Iterator>
		lrb_tree_set_3w(Iterator first, Iterator last, const Compare& comp_, const allocator_type& alloc_ = allocator_type()): the_tree(comp_, alloc_)
		{ the_tree.insert(first, last); }

		lrb_tree_set_3w(const lrb_tree_set_3w& s): the_tree(s.the_tree)
		{}

		lrb_tree_set_3w& operator=(const lrb_tree_set_3w& s)
		{
			the_tree = s.the_tree;
			return *this;
		}
		...
		friend bool operator== (const lrb_tree_set_3w<A, B, C>&, const lrb_tree_set_3w<A, B, C>&);
