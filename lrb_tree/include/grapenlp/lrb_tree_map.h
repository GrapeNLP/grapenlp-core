...
#ifndef GRAPENLP_LRB_TREE_MAP_H
#define GRAPENLP_LRB_TREE_MAP_H
...
namespace grapenlp
{
	...
	public:
    	...
    	{
    		friend class lrb_tree_map<Key, T, Compare, Allocator>;
    	protected:
    		Compare comp;
    ...
    public:
    	lrb_tree_map(): the_tree()
		{}

		explicit lrb_tree_map(const Compare& comp_, const allocator_type& alloc_ = allocator_type()): the_tree(comp_, alloc_)
		{}

		template<typename Iterator>
		lrb_tree_map(Iterator first, Iterator last): the_tree()
		{ the_tree.insert(first, last); }

		template<typename Iterator>
		lrb_tree_map(Iterator first, Iterator last, const Compare& comp_, const allocator_type& alloc_ = allocator_type()): the_tree(comp_, alloc_)
		{ the_tree.insert(first, last); }

		lrb_tree_map(const lrb_tree_map& s): the_tree(s.the_tree)
		{}

		lrb_tree_map& operator=(const lrb_tree_map& s)
		{
			the_tree = s.the_tree;
			return *this;
		}
		...
		mapped_type& at(const key_type& k)
		{
			...
			if (it == end() || key_comp()(k, it->first))
				throw std::out_of_range("lrb_tree_map::at");
			return it->second;
		}
		...
		friend bool operator== (const lrb_tree_map<A, B, C, D>&, const lrb_tree_map<A, B, C, D>&);
	...
	inline bool operator==(const lrb_tree_map<Key, T, Compare, Allocator>& m1, const lrb_tree_map<Key, T, Compare, Allocator>& m2)
	...
	template<typename Key, typename T, typename Compare, typename Allocator>
	inline void swap(lrb_tree_map<Key, T, Compare, Allocator>& m1, lrb_tree_map<Key, T, Compare, Allocator>& m2)
    { m1.swap(m2); }
} //namespace grapenlp

#endif /*GRAPENLP_LRB_TREE_MAP_H*/
