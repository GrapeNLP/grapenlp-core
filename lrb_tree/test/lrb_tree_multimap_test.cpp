...
#include <grape/lrb_tree_to_dot_serializer.h>
...
typedef lrb_tree_multimap<int, int> multimap_type;
...
int main(int argc, char **argv)
{
	...
	if (vm.count("sorted"))
	{
		...
		multimap_type::iterator hint(m.begin());
		for (i = ints_to_insert.begin(); i != ints_to_insert.end(); ++i)
		{
			std::cout << *i << ' ';
			std::cout.flush();
			multimap_type::iterator k(m.insert(hint, *i));
			if (k != m.begin())
			{
				multimap_type::iterator l(k);
				--l;
				if (k != m.begin() && k->first == l->first)
					std::cout << "(repeated) ";
			}
		}
	}
	else
	{
		...
		for (i = ints_to_insert.begin(); i != ints_to_insert.end(); ++i)
		{
			std::cout << *i << ' ';
			std::cout.flush();
			multimap_type::iterator k(m.insert(*i));
			multimap_type::iterator l(k);
			--l;
			if (k != m.begin() && k->first == l->first)
				std::cout << "(repeated) ";
	...
	for (j = bounds.begin(); j != bounds.end(); ++j)
	{
		...
		std::cout.flush();
		multimap_type::iterator it(m.lower_bound(*j));
		if (it == m.end())
			std::cout << " none";
		else std::cout << *it;
		std::cout << " / ";
