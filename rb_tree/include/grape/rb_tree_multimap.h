...
#ifndef GRAPE_RB_TREE_MULTIMAP_H
#define GRAPE_RB_TREE_MULTIMAP_H

#include <stdexcept>
#include <grape/rb_tree.h>

namespace grape
{
	template<typename Key, typename T, typename Compare = std::less<Key>, typename Allocator = std::allocator<std::pair<const Key, T> > >
	class rb_tree_multimap
	{
	public:
    ...
    public:
    	...
    	{
    		friend class rb_tree_multimap<Key, T, Compare, Allocator>;
    	protected:
    		Compare comp;
    ...
    public:
    	rb_tree_multimap(): the_tree()
		{}

		explicit rb_tree_multimap(const Compare& comp_, const allocator_type& alloc_ = allocator_type()): the_tree(comp_, alloc_)
		{}

		template<typename Iterator>
		rb_tree_multimap(Iterator first, Iterator last): the_tree()
		{ the_tree.multi_insert(first, last); }

		template<typename Iterator>
		rb_tree_multimap(Iterator first, Iterator last, const Compare& comp_, const allocator_type& alloc_ = allocator_type()): the_tree(comp_, alloc_)
		{ the_tree.multi_insert(first, last); }

		rb_tree_multimap(const rb_tree_multimap& s): the_tree(s.the_tree)
		{}

		rb_tree_multimap& operator=(const rb_tree_multimap& s)
		{
			the_tree = s.the_tree;
			return *this;
		}
		...
		friend bool operator== (const rb_tree_multimap<A, B, C, D>&, const rb_tree_multimap<A, B, C, D>&);
	...
	inline bool operator==(const rb_tree_multimap<Key, T, Compare, Allocator>& m1, const rb_tree_multimap<Key, T, Compare, Allocator>& m2)
	...
	template<typename Key, typename T, typename Compare, typename Allocator>
	inline void swap(rb_tree_multimap<Key, T, Compare, Allocator>& m1, rb_tree_multimap<Key, T, Compare, Allocator>& m2)
    { m1.swap(m2); }
} //namespace grape

#endif /*GRAPE_RB_TREE_MULTIMAP_H*/
