...
#ifndef GRAPE_RB_TREE_SET_H
#define GRAPE_RB_TREE_SET_H
...
namespace grape
{
    ...
    public:
		...
		rb_tree_set(): the_tree()
		{}

		explicit rb_tree_set(const Compare& comp_, const allocator_type& alloc_ = allocator_type()): the_tree(comp_, alloc_)
		{}

		template<typename Iterator>
		rb_tree_set(Iterator first, Iterator last): the_tree()
		{ the_tree.insert(first, last); }

		template<typename Iterator>
		rb_tree_set(Iterator first, Iterator last, const Compare& comp_, const allocator_type& alloc_ = allocator_type()): the_tree(comp_, alloc_)
		{ the_tree.insert(first, last); }

		rb_tree_set(const rb_tree_set& s): the_tree(s.the_tree)
		{}

		rb_tree_set& operator=(const rb_tree_set& s)
		{
			the_tree = s.the_tree;
			return *this;
		}
		...
		friend bool operator== (const rb_tree_set<A, B, C>&, const rb_tree_set<A, B, C>&);
