...
#ifndef GRAPENLP_RB_TREE_MULTISET_3W_H
#define GRAPENLP_RB_TREE_MULTISET_3W_H

#include <grapenlp/rb_tree_3w.h>

namespace grapenlp
{
	template<typename Key, typename Compare = compare_3w<Key>, typename Allocator = std::allocator<Key> >
	class rb_tree_multiset_3w
	{
    public:
...
*/
    ...
    private:
    	...
    	typedef rb_tree_3w<key_type, value_type, identity<value_type>, key_compare, key_allocator_type> rb_tree_type;
    ...
    public:
		...
		rb_tree_multiset_3w(): the_tree()
		{}

		explicit rb_tree_multiset_3w(const Compare& comp_, const allocator_type& alloc_ = allocator_type()): the_tree(comp_, alloc_)
		{}

		template<typename Iterator>
		rb_tree_multiset_3w(Iterator first, Iterator last): the_tree()
		{ the_tree.multi_insert(first, last); }

		template<typename Iterator>
		rb_tree_multiset_3w(Iterator first, Iterator last, const Compare& comp_, const allocator_type& alloc_ = allocator_type()): the_tree(comp_, alloc_)
		{ the_tree.multi_insert(first, last); }

		rb_tree_multiset_3w(const rb_tree_multiset_3w& s): the_tree(s.the_tree)
		{}

		rb_tree_multiset_3w& operator=(const rb_tree_multiset_3w& s)
		{
			the_tree = s.the_tree;
			return *this;
		}
		...
		void swap(rb_tree_multiset_3w& s)
		...
		friend bool operator== (const rb_tree_multiset_3w<A, B, C>&, const rb_tree_multiset_3w<A, B, C>&);
	...
	inline bool operator==(const rb_tree_multiset_3w<Key, Compare, Allocator>& s1, const rb_tree_multiset_3w<Key, Compare, Allocator>& s2)
	...
	template<typename Key, typename Compare, typename Allocator>
	inline void swap(rb_tree_multiset_3w<Key, Compare, Allocator>& s1, rb_tree_multiset_3w<Key, Compare, Allocator>& s2)
    { s1.swap(s2); }
} //namespace grapenlp

#endif /*GRAPENLP_RB_TREE_MULTISET_3W_H*/
