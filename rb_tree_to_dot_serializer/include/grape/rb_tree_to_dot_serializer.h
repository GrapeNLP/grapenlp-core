...
#ifndef GRAPE_RB_TREE_TO_DOT_SERIALIZER_H
#define GRAPE_RB_TREE_TO_DOT_SERIALIZER_H
#include <ostream>
#include <string>
#include <algorithm>
...
namespace grape
{
	...
	private:
		...
		template<typename CharT, typename Traits>
		inline std::basic_ostream<CharT, Traits>& serialize_subtree(std::basic_ostream<CharT, Traits>& out, base_node_const_ref x)
		{
...
//			out << "size = \"8,5\";" << std::endl;
	...
	inline std::basic_ostream<CharT, Traits>& rb_tree_to_dot_serialize(std::basic_ostream<CharT, Traits>& out, const char *name, const rb_tree<Key, Value, KeyGetter, Compare, Allocator> &t)
	{ return rb_tree_to_dot_serializer<Key, typename tag_serializer<Value>::type, KeyGetter, Compare, Allocator>()(out, name, t); }
	...
	inline std::basic_ostream<CharT, Traits>& rb_tree_to_dot_serialize(std::basic_ostream<CharT, Traits>& out, const char *name, const rb_tree<Key, typename serializer_traits<ValueSerializer>::elem_type, KeyGetter, Compare, Allocator> &t, const ValueSerializer &vs)
	{ return rb_tree_to_dot_serializer<Key, ValueSerializer, KeyGetter, Compare, Allocator>(vs)(out, name, t); }
} //namespace grape

#endif /*GRAPE_RB_TREE_TO_DOT_SERIALIZER_H*/
