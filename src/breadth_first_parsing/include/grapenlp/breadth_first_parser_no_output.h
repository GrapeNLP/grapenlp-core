...
namespace grapenlp
{
	template<typename TagInput, typename TagOutput, typename SourceRef, typename ContextKey, typename ContextValue, typename StateConstRefStackPool, assoc_container_impl_choice execution_state_set_impl_choice>
	struct breadth_first_parser_no_output
	{
		...
		template<typename ExtraInsertOp>
		void process_epsilon_context_transitions(const execution_state &x_s, outgoing_epsilon_context_transition_set_iterator epsilon_context_transition_begin, outgoing_epsilon_context_transition_set_iterator epsilon_context_transition_end, ExtraInsertOp op)
		{
                    ...
                    if (result.second) {
                        ...
                        result.first->serialize(std::wcout) <<
                                                            ...
                                                            *(inserting_context_transition_begin->value) <<
                                                            L"> : " <<
                                                            tag_output_serializer()(std::wcout,
                                                                                    inserting_context_transition_begin->output)
                                                            <<
                                                            L')' <<
                                                            std::endl;
#endif
		...
		template<typename ExtraInsertOp>
		void eclosure(SourceRef in, bool next_token_isnt_white_separated, const context_type &c, ExtraInsertOp op)
		{
		...
		bool operator()(const machine& grammar, SourceRef input_begin, SourceRef input_end, bool hasnt_white_at_begin, bool hasnt_white_at_end, const context_type &c)
		{
			...
			if (input_begin == input_end)
			{
				...
				eclosure(input_begin, c, hasnt_white_at_begin, ack_op);
			}
