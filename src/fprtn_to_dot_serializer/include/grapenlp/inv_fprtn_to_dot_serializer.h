...
#ifndef GRAPENLP_INV_FPRTN_TO_DOT_SERIALIZER_H
#define GRAPENLP_INV_FPRTN_TO_DOT_SERIALIZER_H
#include <ostream>
#include <string>
...
namespace grapenlp
{
	...
	struct inv_fprtn_to_dot_serializer
	{
	...
	public:
		inv_fprtn_to_dot_serializer(): tis()
		{}

		inv_fprtn_to_dot_serializer(const TagInputSerializer &tis_): tis(tis_)
		{}
		...
		std::basic_ostream<CharT, Traits>& serialize_state(std::basic_ostream<CharT, Traits>& out, state_const_ref s, SourceRef src_begin)
		{
			...
			for (outgoing_epsilon_transition_set_const_iterator oet_it(s->incoming_epsilon_transitions.begin()); oet_it != s->incoming_epsilon_transitions.end(); ++oet_it)
			{
				serialize_state_label(out, s);
				out << " -> ";
			...
			for (outgoing_consuming_transition_set_const_iterator oconst_it(s->incoming_consuming_transitions.begin()); oconst_it != s->incoming_consuming_transitions.end(); ++oconst_it)
			{
				serialize_state_label(out, s);
				out << " -> ";
			...
			for (outgoing_call_transition_set_const_iterator ocallt_it(s->incoming_call_transitions.begin()); ocallt_it != s->incoming_call_transitions.end(); ++ocallt_it)
			{
				//Call
				serialize_state_label(out, s);
				out << " -> ";
			...
			for (incoming_filtered_pop_transition_set_const_iterator ifpopt_it(s->incoming_filtered_pop_transitions.begin()); ifpopt_it != s->incoming_filtered_pop_transitions.end(); ++ifpopt_it)
			{
				serialize_state_label(out, s);
				out << " -> ";
...
//			out << "node [shape = ellipse];" << std::endl;
	...
	{ return inv_fprtn_to_dot_serializer<SourceRef, typename tag_serializer_traits<TagInput>::type>()(out, fprtn_name, the_fprtn, src_begin); }
...
#endif /*GRAPENLP_INV_FPRTN_TO_DOT_SERIALIZER_H*/
