...
void u_read_grammar(ualuaw_rtno &grammar, ual_trie &ualt, ua_trie &uat, tolower_u_text_delaf &dico, u_context_key_value_hasher &c_hasher)
{
	FILE *f(u_fopen("../Data/Unitex/Spanish/Graphs/wdico_test.fst2", U_READ));
	if (f == NULL)
		fatal_error("Unable to open grammar file to read\n");
	ualuaw_fst2_reader()(f, grammar, ualt, uat, dico, c_hasher);
	u_fclose(f);
}
