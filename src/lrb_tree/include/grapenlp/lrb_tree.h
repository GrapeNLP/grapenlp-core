...
#ifndef GRAPENLP_LRB_TREE_H
#define GRAPENLP_LRB_TREE_H
...
namespace grapenlp
{
	...
	class lrb_tree
	{
		...
		friend struct lrb_tree_to_dot_serializer;
#endif
	public:
...
//			friend class node;
	...
	protected:
		...
		void copy_tree(const lrb_tree &t)
		{
		...
		void copy_left_subtree(const lrb_tree &t, base_node_const_ref tsubroot_ref, base_node_ref subroot_ref)
		{
		...
		void copy_right_subtree(const lrb_tree &t, base_node_const_ref tsubroot_ref, base_node_ref subroot_ref)
		{
			if (t.has_left(tsubroot_ref))
			{
...
*/
	...
	public:
		lrb_tree(const KeyCompare& comp_ = KeyCompare(), const allocator_type& alloc_ = allocator_type()): d(comp_, alloc_)
		{}

		lrb_tree(const lrb_tree &t): d(t.d.comp, t.get_node_allocator())
		{
			if (!t.empty())
			{
				copy_tree(t);
		...
		lrb_tree& operator= (const lrb_tree& t)
		{
			if (this != &t)
			{
				clear();
	...
	private:
		bool compare_value_keys(const value_type &v1, const value_type &v2) const
		{ return d.comp(KeyGetter()(v1), KeyGetter()(v2)); }

		bool compare_value_keys(const value_type &v, base_node_const_ref x) const
		{ return d.comp(KeyGetter()(v), KeyGetter()(x->value())); }

		bool compare_value_keys(base_node_const_ref x, const value_type &v) const
		{ return d.comp(KeyGetter()(x->value()), KeyGetter()(v)); }

		bool compare_keys(const key_type &k1, const key_type &k2) const
		{ return d.comp(k1, k2); }

		bool compare_keys(const key_type &k, base_node_const_ref x) const
		{ return d.comp(k, KeyGetter()(x->value())); }

		bool compare_keys(base_node_const_ref x, const key_type &k) const
	...
	private:
		...
		std::pair<base_node_ref, bool> insert(base_node_const_ref p, const value_type& v)
		{
			...
			if (is_nil(p))
			{
				...
				if (!empty() && compare_value_keys(last_base_node_const_ref(), v))
			...
			if (compare_value_keys(v, p))
			{
				...
				//--p < v < p
				if (compare_value_keys(p->previous_ref, v))
			    {
					//--p has no right child: --p under left of p, --p has no right child, insert as --p right child
...
//				more expensive if the data is smaller than the set of pointers to modify
...
//				z->key() = y->key();
	...
	public:
		...
		bool operator== (const lrb_tree& t) const
	...
	void swap(lrb_tree<Key, Value, KeyGetter, KeyCompare>& x, lrb_tree<Key, Value, KeyGetter, KeyCompare>& y)
	{ x.swap(y); }

    template<typename Key, typename Value, typename KeyGetter, typename KeyCompare, typename Allocator>
    typename lrb_tree<Key, Value, KeyGetter, KeyCompare, Allocator>::value_type& lrb_tree<Key, Value, KeyGetter, KeyCompare, Allocator>::base_node::value()
    { return static_cast<node_ref>(this)->v; }

    template<typename Key, typename Value, typename KeyGetter, typename KeyCompare, typename Allocator>
    const typename lrb_tree<Key, Value, KeyGetter, KeyCompare, Allocator>::value_type& lrb_tree<Key, Value, KeyGetter, KeyCompare, Allocator>::base_node::value() const
    { return static_cast<node_const_ref>(this)->v; }
} //namespace grapenlp

#endif /*GRAPENLP_LRB_TREE_H*/
