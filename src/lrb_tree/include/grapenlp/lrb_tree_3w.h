...
#ifndef GRAPENLP_LRB_TREE_3W_H
#define GRAPENLP_LRB_TREE_3W_H
...
namespace grapenlp
{
	...
	struct lrb_tree_3w_to_dot_serializer;
#endif
	...
	class lrb_tree_3w
	{
		...
		friend struct lrb_tree_3w_to_dot_serializer;
#endif
	public:
...
//			friend class node;
	...
	protected:
		...
		void copy_tree(const lrb_tree_3w &t)
		{
		...
		void copy_left_subtree(const lrb_tree_3w &t, base_node_const_ref tsubroot_ref, base_node_ref subroot_ref)
		{
		...
		void copy_right_subtree(const lrb_tree_3w &t, base_node_const_ref tsubroot_ref, base_node_ref subroot_ref)
		{
			if (t.has_left(tsubroot_ref))
			{
...
*/
	...
	public:
		lrb_tree_3w(const KeyCompare3Way& comp_ = KeyCompare3Way(), const allocator_type& alloc_ = allocator_type()): d(comp_, alloc_)
		{}

		lrb_tree_3w(const lrb_tree_3w &t): d(t.d.comp, t.get_node_allocator())
		{
			if (!t.empty())
			{
				copy_tree(t);
		...
		lrb_tree_3w& operator= (const lrb_tree_3w& t)
		{
			if (this != &t)
			{
				clear();
	...
	public:
		key_compare_3w key_comp() const
		{ return d.comp; }

	private:
		std::ptrdiff_t compare_value_keys_3w(const value_type &v1, const value_type &v2) const
		{ return d.comp(KeyGetter()(v1), KeyGetter()(v2)); }

		std::ptrdiff_t compare_value_keys_3w(const value_type &v, base_node_const_ref x) const
		{ return d.comp(KeyGetter()(v), KeyGetter()(x->value())); }

		std::ptrdiff_t compare_value_keys_3w(base_node_const_ref x, const value_type &v) const
		{ return d.comp(KeyGetter()(x->value()), KeyGetter()(v)); }

		std::ptrdiff_t compare_keys_3w(const key_type &k1, const key_type &k2) const
		{ return d.comp(k1, k2); }

		std::ptrdiff_t compare_keys_3w(base_node_const_ref x, const key_type &k) const
		{ return d.comp(KeyGetter()(x->value()), k); }

		std::ptrdiff_t compare_keys_3w(const key_type &k, base_node_const_ref x) const
		...
		bool has_left(base_node_const_ref x) const
		{ return is_not_nil(x->left_ref); }

		bool has_no_left(base_node_const_ref x) const
		{ return is_nil(x->left_ref); }

		bool has_right(base_node_const_ref x) const
		{ return is_not_nil(x->right_ref); }

		bool has_no_right(base_node_const_ref x) const
		{ return is_nil(x->right_ref); }

		bool has_next(base_node_const_ref x) const
		{ return is_not_nil(x->next_ref); }

		bool has_no_next(base_node_const_ref x) const
		{ return is_nil(x->next_ref); }

		bool has_previous(base_node_const_ref x) const
		{ return is_not_nil(x->previous_ref); }

		bool has_no_previous(base_node_const_ref x) const
		{ return is_nil(x->previous_ref); }

		bool has_missing_child(base_node_const_ref x) const
		{ return has_no_left(x) || has_no_right(x); }

		bool has_both_children(base_node_const_ref x) const
		{ return has_left(x) && has_right(x); }

		bool is_root(base_node_const_ref x) const
		{ return is_nil(x->parent_ref); }

		bool is_leaf(base_node_const_ref x) const
		{ return x->has_no_left && x->has_no_right; }

		bool is_inner(base_node_const_ref x) const
		{ return x->has_left || x->has_right; }

		bool is_left_child(base_node_const_ref x) const
		{ return x == x->parent_ref->left_ref; }

		bool is_right_child(base_node_const_ref x) const
...
*/
		value_type& front()
		{ return first_base_node_ref()->value(); }

		value_type& front() const
		{ return first_base_node_const_ref()->value(); }

		value_type& back()
		{ return last_base_node_ref()->value(); }

		value_type& back() const
		{ return last_base_node_const_ref()->value(); }
	...
	private:
		...
		base_node_ref insert_with_default_mapped_value_as_root(const key_type& k)
		{ return insert_as_root(value_type(k, typename value_type::second_type())); }
		...
		base_node_ref insert_with_default_mapped_value_as_left_child_of(base_node_ref x, const key_type& k)
		{ return insert_as_left_child_of(x, value_type(k, typename value_type::second_type())); }
		...
		base_node_ref insert_with_default_mapped_value_as_right_child_of(base_node_ref x, const key_type& k)
		{ return insert_as_right_child_of(x, value_type(k, typename value_type::second_type())); }
		...
		std::pair<base_node_ref, bool> insert_to_base_node_ref(const value_type &value)
		{
			...
			while (true)
			{
				i = compare_value_keys_3w(value, x);
				if (i < 0)
				{
					if (has_no_left(x))
		...
		//Insertion without duplicates (uses "less than" comparator); the mapped type is constructed iff
		//the insertion is successful; in that case, the default constructor is used
		std::pair<base_node_ref, bool> insert_to_base_node_ref_with_default_mapped_value(const key_type &key)
		{
			if (empty())
				return std::make_pair(insert_with_default_mapped_value_as_root(key), true);
			base_node_ref x(root_ref());

			std::ptrdiff_t i;
			while (true)
			{
				i = compare_keys_3w(key, x);
				if (i < 0)
				{
					if (has_no_left(x))
						return std::make_pair(insert_with_default_mapped_value_as_left_child_of(x, key), true);
					x = x->left_ref;
					continue;
				}
				if (i > 0)
				{
					if (has_no_right(x))
						return std::make_pair(insert_with_default_mapped_value_as_right_child_of(x, key), true);
					x = x->right_ref;
					continue;
				}
				return std::make_pair(x, false);
		...
		base_node_ref multi_insert_to_base_node_ref(const value_type &value)
		{
			...
			while (true)
			{
				if (compare_3w_value_keys(value, x) < 0)
				{
					if (has_no_left(x))
		...
		//Insertion with duplicates (uses "greater or equal than" comparator); the mapped type is constructed iff
		//the insertion is successful; in that case, the default constructor is used
		base_node_ref multi_insert_to_base_node_ref_with_default_mapped_value(const key_type &key)
		{
			if (empty())
				return insert_as_root_with_default_mapped_value(key);
			base_node_ref x(root_ref());

			while (true)
			{
				if (compare_keys_3w(key, x) < 0)
				{
					if (has_no_left(x))
						return insert_with_default_mapped_value_as_left_child_of(x, key);
					x = x->left_ref;
					continue;
				}
				if (has_no_right(x))
					return insert_with_default_mapped_value_as_right_child_of(x, key);
				x = x->right_ref;
			}
		}
	...
	public:
		...
		//Insertion without duplicates (uses "less than" comparator)
		std::pair<iterator, bool> insert_with_default_mapped_value(const key_type &key)
		{
			std::pair<base_node_ref, bool> result(insert_to_base_node_ref_with_default_mapped_value(key));
			return std::make_pair(iterator(result.first), result.second);
		}
		...
		//Insertion with duplicates (uses "greater or equal than" comparator)
		iterator multi_insert_with_default_mapped_value(const key_type &key)
		{ return iterator(multi_insert_to_base_node_ref_with_default_mapped_value(key)); }
	...
	private:
		...
		std::pair<base_node_ref, bool> insert(base_node_const_ref p, const value_type& v)
		{
			...
			if (is_nil(p))
			{
				...
				if (!empty() && compare_value_keys_3w(last_base_node_const_ref(), v) < 0)
			...
			//p < v
			std::ptrdiff_t i(compare_value_keys_3w(p, v));
			if (i < 0)
			{
				...
				if (p == last_base_node_const_ref())
					return std::make_pair(insert_as_right_child_of(last_base_node_ref(), v), true);
				//p < v < ++p
				if (compare_value_keys_3w(v, p->next_ref))
				{
			...
			if (i > 0)
			{
				...
				//--p < v < p
				if (compare_value_keys_3w(p->previous_ref, v))
			    {
					//--p has no right child: --p under left of p, --p has no right child, insert as --p right child
		...
		//Insert with hint: start search for insertion position from p instead of root
		std::pair<base_node_ref, bool> insert_with_default_mapped_value(base_node_const_ref p, const key_type& k)
		{
			//p == nil
			if (is_nil(p))
			{
				//last < v: insert as last's right child
				if (!empty() && compare_keys(last_base_node_const_ref(),	k))
					return std::make_pair(insert_with_default_mapped_value_as_right_child_of(last_base_node_ref(), k), true);
				//empty || v < last: normal insert
				return insert_with_default_mapped_value_to_base_node_ref(k);
			}
			std::ptrdiff_t i(compare_keys_3w(p, k));
			//p < v
			if (i < 0)
			{
				//p == last: insert as last's right child
				if (p == last_base_node_const_ref())
					return std::make_pair(insert_with_default_mapped_value_as_right_child_of(last_base_node_ref(), k), true);
				//p < v < ++p
				if (compare_keys(k, p->next_ref))
				{
					//p has no right child: p under left of ++p, p has no right child, insert as p right child
					if (has_no_right(p))
						return std::make_pair(insert_with_default_mapped_value_as_right_child_of(const_cast<base_node_ref>(p), k), true);
					//p has right child: ++p under right p, ++p has no left child, insert as ++p left child
					return std::make_pair(insert_with_default_mapped_value_as_left_child_of(p->next_ref, k), true);
				}
				//p < ++p < v: normal insert
				return insert_to_base_node_ref_with_default_mapped_value(k);
			}
			//v < p
			if (i > 0)
			{
				//p == first: insert as first's left child
				if (p == first_base_node_const_ref())
					return std::make_pair(insert_with_default_mapped_value_as_left_child_of(first_base_node_ref(), k), true);
				//--p < v < p
				if (compare_keys(p->previous_ref, k))
			    {
					//--p has no right child: --p under left of p, --p has no right child, insert as --p right child
					if (has_no_right(p->previous_ref))
						return std::make_pair(insert_with_default_mapped_value_as_right_child_of(p->previous_ref, k), true);
					//--p has right child: p under right of --p, p has no left child, insert as p left child
					return std::make_pair(insert_with_default_mapped_value_as_left_child_of(const_cast<base_node_ref>(p), k), true);
			    }
				//v < --p < p: normal insert
				return insert_with_default_mapped_value_to_base_node_ref(k);
			}
		...
		base_node_ref multi_insert(base_node_const_ref p, const value_type& v)
		{
			...
			//v < p
			if (compare_value_keys_3w(v, p) < 0)
			{
				//p == first: insert as first's left child
				if (p == first_base_node_const_ref())
					return insert_as_left_child_of(first_base_node_ref(), v);
				//--p < v < p
				if (compare_value_keys_3w(p->previous_ref, v) < 0)
			    {
					//--p has no right child: --p under left of p, --p has no right child, insert as --p right child
					if (has_no_right(p->previous_ref))
			...
			if (p == last_base_node_const_ref())
				return insert_as_right_child_of(last_base_node_ref(), v);
			//p <= v < ++p
			if (compare_value_keys_3w(v, p->next_ref) < 0)
			{
				//p has no right child: p under left of ++p, p has no right child, insert as p right child
				if (has_no_right(p))
		...
		//Insertion with duplicates and hint: use "greater or equal than" comparator and
		//start search for insertion position from p instead of root
		base_node_ref multi_insert_with_default_mapped_value(base_node_const_ref p, const key_type& k)
		{
			//p == nil
			if (is_nil(p))
			{
				//last < v: insert as last's right child
				if (!empty() && compare_keys(last_base_node_const_ref(), k))
					return insert_with_default_mapped_value_as_right_child_of(last_base_node_ref(), k);
				//empty || v < last: normal insert
				return multi_insert_to_base_node_ref_with_default_mapped_value(k);
			}
			//v < p
			if (compare_keys_3w(k, p) < 0)
			{
				//p == first: insert as first's left child
				if (p == first_base_node_const_ref())
					return insert_with_default_mapped_value_as_left_child_of(first_base_node_ref(), k);
				//--p < v < p
				if (compare_keys_3w(p->previous_ref, k) < 0)
			    {
					//--p has no right child: --p under left of p, --p has no right child, insert as --p right child
					if (has_no_right(p->previous_ref))
						insert_with_default_mapped_value_as_right_child_of(p->previous_ref, k);
					//--p has right child: p under right of --p, p has no left child, insert as p left child
					return insert_with_default_mapped_value_as_left_child_of(const_cast<base_node_ref>(p), k);
			    }
				//v < --p <= p: normal insert
				return multi_insert_with_default_mapped_value_to_base_node_ref(k);
			}
			//p <= v
			//p == last: insert as last's right child
			if (p == last_base_node_const_ref())
				return insert_with_default_mapped_value_as_right_child_of(last_base_node_ref(), k);
			//p <= v < ++p
			if (compare_keys_3w(k, p->next_ref) < 0)
			{
				//p has no right child: p under left of ++p, p has no right child, insert as p right child
				if (has_no_right(p))
					return insert_with_default_mapped_value_as_right_child_of(const_cast<base_node_ref>(p), k);
				//p has right child: ++p under right p, ++p has no left child, insert as ++p left child
				return insert_with_default_mapped_value_as_left_child_of(p->next_ref, k);
			}
			//p <= ++p <= v: normal insert
			return multi_insert_to_base_node_ref_with_default_mapped_value(k);
		}
		...
		template<typename Iterator>
		void insert_with_default_mapped_value(Iterator first, Iterator last)
		{
			if (first == last)
				return;
			base_node_ref p(insert(*first).first);
			for (++first; first != last; ++first)
				p = insert_with_default_mapped_value(p, *first);
		}
	...
	public:
		...
		std::pair<iterator, bool> insert_with_default_mapped_value(const_iterator position, const key_type& k)
		{
			std::pair<base_node_ref, bool> result(insert_with_default_mapped_value(position.x, k));
			return std::make_pair(iterator(result.first), result.second);
		}
		...
		iterator multi_insert_with_default_mapped_value(const_iterator position, const key_type& k)
		{ return iterator(multi_insert_with_default_mapped_value(position.x, k)); }
...
//				z->key() = y->key();
	...
	public:
		...
		bool operator== (const lrb_tree_3w& t) const
	...
	void swap(lrb_tree_3w<Key, Value, KeyGetter, KeyCompare3Way>& x, lrb_tree_3w<Key, Value, KeyGetter, KeyCompare3Way>& y)
	{ x.swap(y); }

	template<typename Key, typename Value, typename KeyGetter, typename KeyCompare, typename Allocator>
	typename lrb_tree_3w<Key, Value, KeyGetter, KeyCompare, Allocator>::value_type& lrb_tree_3w<Key, Value, KeyGetter, KeyCompare, Allocator>::base_node::value()
	{ return static_cast<node_ref>(this)->v; }

	template<typename Key, typename Value, typename KeyGetter, typename KeyCompare, typename Allocator>
	const typename lrb_tree_3w<Key, Value, KeyGetter, KeyCompare, Allocator>::value_type& lrb_tree_3w<Key, Value, KeyGetter, KeyCompare, Allocator>::base_node::value() const
	{ return static_cast<node_const_ref>(this)->v; }
} //namespace grapenlp

#endif /*GRAPENLP_LRB_TREE_3W_H*/
