...
#ifndef GRAPENLP_LRB_TREE_MULTIMAP_H
#define GRAPENLP_LRB_TREE_MULTIMAP_H
...
namespace grapenlp
{
    ...
    public:
    	...
    	{
    		friend class lrb_tree_multimap<Key, T, Compare, Allocator>;
    	protected:
    		Compare comp;
    ...
    private:
		...
		typedef lrb_tree<key_type, value_type, pair_first<const Key, T>, key_compare, value_allocator_type> lrb_tree_type;
    ...
    public:
    	lrb_tree_multimap(): the_tree()
		{}

		explicit lrb_tree_multimap(const Compare& comp_, const allocator_type& alloc_ = allocator_type()): the_tree(comp_, alloc_)
		{}

		template<typename Iterator>
		lrb_tree_multimap(Iterator first, Iterator last): the_tree()
		{ the_tree.multi_insert(first, last); }

		template<typename Iterator>
		lrb_tree_multimap(Iterator first, Iterator last, const Compare& comp_, const allocator_type& alloc_ = allocator_type()): the_tree(comp_, alloc_)
		{ the_tree.multi_insert(first, last); }

		lrb_tree_multimap(const lrb_tree_multimap& s): the_tree(s.the_tree)
		{}

		lrb_tree_multimap& operator=(const lrb_tree_multimap& s)
		{
			the_tree = s.the_tree;
			return *this;
		}
		...
		{ return the_tree.multi_insert_with_default_mapped_value(k); }

		iterator insert(iterator position, const value_type& v)
		{ return the_tree.multi_insert(position, v); }

		iterator insert(iterator position, const key_type& k)
		{ return the_tree.multi_insert_with_default_mapped_value(position, k); }
		...
		{ return the_tree.unbalanced_multi_erase(k); }
		...
		friend bool operator== (const lrb_tree_multimap<A, B, C, D>&, const lrb_tree_multimap<A, B, C, D>&);
	...
	inline bool operator==(const lrb_tree_multimap<Key, T, Compare, Allocator>& m1, const lrb_tree_multimap<Key, T, Compare, Allocator>& m2)
	...
	template<typename Key, typename T, typename Compare, typename Allocator>
	inline void swap(lrb_tree_multimap<Key, T, Compare, Allocator>& m1, lrb_tree_multimap<Key, T, Compare, Allocator>& m2)
    { m1.swap(m2); }
} //namespace grapenlp

#endif /*GRAPENLP_LRB_TREE_MULTIMAP_H*/
