...
#ifndef GRAPENLP_LRB_TREE_MULTISET_H
#define GRAPENLP_LRB_TREE_MULTISET_H
...
*/
    ...
    public:
		...
		lrb_tree_multiset(): the_tree()
		{}

		explicit lrb_tree_multiset(const Compare& comp_, const allocator_type& alloc_ = allocator_type()): the_tree(comp_, alloc_)
		{}

		template<typename Iterator>
		lrb_tree_multiset(Iterator first, Iterator last): the_tree()
		{ the_tree.multi_insert(first, last); }

		template<typename Iterator>
		lrb_tree_multiset(Iterator first, Iterator last, const Compare& comp_, const allocator_type& alloc_ = allocator_type()): the_tree(comp_, alloc_)
		{ the_tree.multi_insert(first, last); }

		lrb_tree_multiset(const lrb_tree_multiset& s): the_tree(s.the_tree)
		{}

		lrb_tree_multiset& operator=(const lrb_tree_multiset& s)
		{
			the_tree = s.the_tree;
			return *this;
		}
		...
		friend bool operator== (const lrb_tree_multiset<A, B, C>&, const lrb_tree_multiset<A, B, C>&);
	...
	inline bool operator==(const lrb_tree_multiset<Key, Compare, Allocator>& s1, const lrb_tree_multiset<Key, Compare, Allocator>& s2)
	...
	template<typename Key, typename Compare, typename Allocator>
	inline void swap(lrb_tree_multiset<Key, Compare, Allocator>& s1, lrb_tree_multiset<Key, Compare, Allocator>& s2)
    { s1.swap(s2); }
} //namespace grapenlp

#endif /*GRAPENLP_LRB_TREE_MULTISET_H*/
