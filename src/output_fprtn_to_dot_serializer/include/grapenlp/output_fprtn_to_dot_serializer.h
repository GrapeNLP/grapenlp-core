...
#ifndef GRAPENLP_OUTPUT_FPRTN_TO_DOT_SERIALIZER_H
#define GRAPENLP_OUTPUT_FPRTN_TO_DOT_SERIALIZER_H
#include <ostream>
#include <string>
...
namespace grapenlp
{
	...
	public:
		output_fprtn_to_dot_serializer(): tis()
		{}

		output_fprtn_to_dot_serializer(const TagInputSerializer &tis_): tis(tis_)
		{}
		...
		std::basic_ostream<CharT, Traits>& serialize_state(std::basic_ostream<CharT, Traits>& out, const state &s, std::size_t i)
		{
			...
			for (outgoing_epsilon_transition_set_const_iterator oet_it(s.second.outgoing_epsilon_transitions.begin()); oet_it != s.second.outgoing_epsilon_transitions.end(); ++oet_it)
			{
				serialize_state_label(out, s);
				out << " -> ";
			...
			for (outgoing_consuming_transition_set_const_iterator oconst_it(s.second.outgoing_consuming_transitions.begin()); oconst_it != s.second.outgoing_consuming_transitions.end(); ++oconst_it)
			{
				serialize_state_label(out, s);
				out << " -> ";
			...
			for (outgoing_call_transition_set_const_iterator ocallt_it(s.second.outgoing_call_transitions.begin()); ocallt_it != s.second.outgoing_call_transitions.end(); ++ocallt_it)
			{
				//Call
				serialize_state_label(out, s);
				out << " -> ";
			...
			for (outgoing_filtered_pop_transition_set_const_iterator ofpopt_it(s.second.outgoing_filtered_pop_transitions.begin()); ofpopt_it != s.second.outgoing_filtered_pop_transitions.end(); ++ofpopt_it)
			{
				serialize_state_label(out, s);
				out << " -> ";
...
//			out << "node [shape = ellipse];" << std::endl;
			...
			out << "ini" << " -> ";
			serialize_state_label(out, m.initial_state);
			out << ';' << std::endl;
	...
	{ return output_fprtn_to_dot_serializer<ContextMask, SourceRef, TagInputSerializer, RTNOTagInput, execution_state_set_impl_choice, edc, StateMappedExtraData>(tis)(out, output_fprtn_name, the_output_fprtn); }
	...
	void write_fprtn_dot(const char *fprtn_name, const char *dot_fprtn_pathname, output_fprtn<ContextMask, SourceRef, TagInput, RTNOTagInput, execution_state_set_impl_choice, edc, StateMappedExtraData> &m)
	{
		std::wofstream fout;
		fout.imbue(std::locale(setlocale(LC_CTYPE, NULL)));
		fout.open(dot_fprtn_pathname);
		if (fout)
		{
			output_fprtn_to_dot_serialize(fout, fprtn_name, m);
			fout.close();
		}
		else std::wcerr << "Unable to open file \"" << dot_fprtn_pathname << "\" for writing" << std::endl;
		std::wcout << L"Written \"" << dot_fprtn_pathname << "\"\n";
	}
#endif

} //namespace grapenlp

#endif /*GRAPENLP_OUTPUT_FPRTN_TO_DOT_SERIALIZER_H*/
