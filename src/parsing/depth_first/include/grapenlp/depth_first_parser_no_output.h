...
namespace grapenlp
{
	...
	struct depth_first_parser_no_output
	{
		...
		bool traverse(execution_state &x_s, source_ref input_end, bool next_token_isnt_white_separated, bool hasnt_white_at_end, const u_context &c)
		{
            ...
            for (; oect_it != x_s.q->outgoing_epsilon_context_transitions.end(); ++oect_it)
            {
                if (oect_it->mask.match(c))
                {
                    ...
                    x_t.serialize(std::wcout) << L" (";
                    oect_it->mask.wserialize_context_expression(std::wcout) << L" : <E>)" << std::endl;
#else
            ...
            for (; oict_it != x_s.q->outgoing_inserting_context_transitions.end(); ++oict_it)
            {
                if (oict_it->mask.match(c))
                {
                    ...
                    x_t.serialize(std::wcout) << L" (";
                    oict_it->mask.wserialize_context_expression(std::wcout) << L" : ";
