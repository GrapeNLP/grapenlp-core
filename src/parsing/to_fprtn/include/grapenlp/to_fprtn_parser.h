...
#ifndef GRAPENLP_TO_FPRTN_PARSER_H
#define GRAPENLP_TO_FPRTN_PARSER_H
...
namespace grapenlp
{
	...
	struct to_fprtn_parser
	{
		...
		typedef output_fprtn<u_context_mask, SourceRef, TagOutput, TagInput, execution_state_set_impl_choice> out_machine;
		...
		struct acceptor_execution_state_inserter
		{
			...
			acceptor_execution_state_inserter(out_state &global_acceptor_state_, const in_state_const_ref in_initial_state_): global_acceptor_state(global_acceptor_state_), in_initial_state(in_initial_state_)
			{}
			...
			void operator()(out_state &x)
			{
				...
				{
					x.add_filtered_pop_transition(&global_acceptor_state);
#ifdef TRACE
					std::wcout << L"added filtered pop transition from " << x.second.wlabel() << L" to global acceptor state" << std::endl;
#endif
				}
			}
		};
		...
		to_fprtn_parser(): input_match(), e(), dummy_x_p_key(NULL, NULL, NULL, NULL, 0), dummy_e_comp(NULL, NULL, NULL)
		{}
		to_fprtn_parser(match input_match_): input_match(input_match_), e(), dummy_x_p_key(NULL, NULL, NULL, NULL, 0), dummy_e_comp(NULL, NULL, NULL)
		{}
...
//					static_cast<out_state&>(*result.first).set_mapped(in);
		...
		template<typename ExtraInsertOp>
		void process_inserting_context_transitions(out_state &x_s, in_outgoing_inserting_context_transition_set_iterator inserting_context_transition_begin, in_outgoing_inserting_context_transition_set_iterator inserting_context_transition_end, chart_item &v, SourceRef in, out_machine &out, ExtraInsertOp op)
		{
...
//				std::pair<out_state_map_iterator, bool> result(v.create_other_state(static_cast<in_state_const_ref>(inserting_context_transition_begin->target), x_s.first.q_h, x_s.first.i));
...
//					static_cast<out_state&>(*result.first).set_mapped(in);
		...
		template<typename ExtraInsertOp>
		void eclosure(std::size_t idx, SourceRef in, bool next_token_isnt_white_separated, out_machine& out, ExtraInsertOp op)
		{
...
//								static_cast<out_state&>(*x_t_result.first).set_mapped(in);
								...
								if (x_p_result.first->second.is_filtered_popping_transition_creator)
									for (;e_comp_it != t.end() && e_comp_it->q_c == call_transition_begin->called; ++e_comp_it)
										e_comp_it->r_f->add_filtered_pop_transition(static_cast<out_state_ref>(&(*x_t_result.first)));
								e.push(static_cast<out_state_ref>(&(*x_t_result.first)));
								op(static_cast<out_state&>(*x_t_result.first));
							}
							else
...
//								static_cast<out_state&>(*x_t_result.first).set_mapped(in);
								...
								if (x_p_it->second.is_filtered_popping_transition_creator)
									x_s_ref->add_filtered_pop_transition(static_cast<out_state_ref>(&(*x_t_result.first)));
								e.push(static_cast<out_state_ref>(&(*x_t_result.first)));
								op(static_cast<out_state&>(*x_t_result.first));
							}
							else
...
//			static_cast<out_state&>(*result.first).set_mapped(out.initial_state.second.src_ref);
		...
		void operator()(const in_machine& grammar, bool hasnt_white_at_begin, bool hasnt_white_at_end, const u_context &c, out_machine& out)
		{
#ifdef TRACE
			...
			acceptor_execution_state_inserter ins_op(out.global_acceptor_state, grammar.initial_state());
			...
			if (input_begin == input_end)
			{
				...
				eclosure(0, input_begin, hasnt_white_at_begin, out, ins_op);
			}
			...
			else
			{
				...
				//First token is white separated if there are trailing whites at the beginning
				eclosure(0, input_begin, hasnt_white_at_begin, out, no_op);
				...
				while (!out.the_chart[idx].other_states.empty() && idx < last_idx)
				{
					...
					++idx;
					eclosure(idx, input_begin_plus_1, (*input_begin)->end == (*input_begin_plus_1)->begin, out, no_op);
					++input_begin;
				}
				...
				if (!out.the_chart[idx].other_states.empty())
				{
					...
					eclosure(idx, input_begin_plus_1, hasnt_white_at_end, out, ins_op);
					++input_begin;
				}
			}
#ifdef TRACE
			std::wcout << L"***** Paused ESs *****\n";
			std::wcout << L"----- V[" << static_cast<int>(0) << "] -----\n";
			paused_execution_state_map_const_iterator x_p_it(out.the_chart[0].pess.begin());
			for (paused_execution_state_map_const_iterator x_p_it(out.the_chart[0].pess.begin()); x_p_it != out.the_chart[0].pess.end(); ++x_p_it)
...
#endif /*GRAPENLP_TO_FPRTN_PARSER_H*/
