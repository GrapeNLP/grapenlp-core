...
namespace grapenlp
{
	...
	struct to_fprtn_zpps_parser
	{
		...
		typedef typename output_fprtn_zpps<u_context_mask, SourceRef, TagOutput, TagInput, execution_state_set_impl_choice>::type out_machine;
...
//					static_cast<out_state&>(*result.first).set_mapped(in);
					...
					static_cast<out_state&>(*result.first).serialize_earley_exe_state(std::wcout) <<
					    L" (" <<
					    inserting_transition_type <<
					    L"<E> : ";
					tag_output_serializer()(std::wcout, inserting_transition_begin->output) <<
					    L')' <<
					    std::endl;
#endif
					e.push(static_cast<out_state_ref>(&(*result.first)));
