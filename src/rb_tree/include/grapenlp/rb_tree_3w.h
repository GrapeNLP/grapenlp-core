...
*/
	...
	private:
		std::ptrdiff_t compare_3w(const value_type &v1, const value_type &v2) const
		{ return d.comp(KeyGetter()(v1), KeyGetter()(v2)); }

		std::ptrdiff_t compare_3w(const key_type &k, base_node_const_ref x) const
		{ return d.comp(k, KeyGetter()(x->value())); }

		std::ptrdiff_t compare_3w(base_node_const_ref x, const key_type &k) const
...
//				z->key() = y->key();
	...
	public:
		...
		bool operator== (const rb_tree_3w& t) const
	...
	void swap(rb_tree_3w<Key, Value, KeyGetter, KeyCompare3Way>& x, rb_tree_3w<Key, Value, KeyGetter, KeyCompare3Way>& y)
	{ x.swap(y); }
} //namespace grapenlp
