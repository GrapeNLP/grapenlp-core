...
#ifndef GRAPENLP_RB_TREE_MAP_3W_H
#define GRAPENLP_RB_TREE_MAP_3W_H
...
namespace grapenlp
{
    ...
    public:
    	...
    	{
    		friend class rb_tree_map_3w<Key, T, Compare, Allocator>;
    	protected:
    		Compare comp;
    ...
    public:
    	rb_tree_map_3w(): the_tree()
		{}

		explicit rb_tree_map_3w(const Compare& comp_, const allocator_type& alloc_ = allocator_type()): the_tree(comp_, alloc_)
		{}

		template<typename Iterator>
		rb_tree_map_3w(Iterator first, Iterator last): the_tree()
		{ the_tree.insert(first, last); }

		template<typename Iterator>
		rb_tree_map_3w(Iterator first, Iterator last, const Compare& comp_, const allocator_type& alloc_ = allocator_type()): the_tree(comp_, alloc_)
		{ the_tree.insert(first, last); }

		rb_tree_map_3w(const rb_tree_map_3w& s): the_tree(s.the_tree)
		{}

		rb_tree_map_3w& operator=(const rb_tree_map_3w& s)
		{
			the_tree = s.the_tree;
			return *this;
		}
		...
		mapped_type& at(const key_type& k)
		{
			iterator it(lower_bound(k));
			if (it == end() || key_comp()(k, it->first))
				throw std::out_of_range("rb_tree_map_3w::at");
			return it->second;
		}
		...
		friend bool operator== (const rb_tree_map_3w<A, B, C, D>&, const rb_tree_map_3w<A, B, C, D>&);
	...
	inline bool operator==(const rb_tree_map_3w<Key, T, Compare, Allocator>& m1, const rb_tree_map_3w<Key, T, Compare, Allocator>& m2)
	...
	template<typename Key, typename T, typename Compare, typename Allocator>
	inline void swap(rb_tree_map_3w<Key, T, Compare, Allocator>& m1, rb_tree_map_3w<Key, T, Compare, Allocator>& m2)
    { m1.swap(m2); }
} //namespace grapenlp

#endif /*GRAPENLP_RB_TREE_MAP_3W_H*/
