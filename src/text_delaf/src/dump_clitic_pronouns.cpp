...
int main(int argc, char **argv)
{
	...
	u_read_dico(d);
	std::wcout << L"Writing clitic pronouns" << std::endl;

	FILE *f(u_fopen("../clitic_pronouns.txt", U_WRITE));
	if (f == NULL)
		fatal_error("Unable to open file clitic_pronouns.txt for writing\n");

	u_set_trie::const_iterator i(d.clitic_pronouns.begin());
	++i;
	while (i != d.clitic_pronouns.end())
	{
		if (i->data)
		{
