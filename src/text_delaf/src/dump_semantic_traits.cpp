...
int main(__attribute__((unused)) int argc, __attribute__((unused)) char **argv)
{
	...
	u_read_dico(d);
	std::wcout << L"Writing semantic traits" << std::endl;

	FILE *f(u_fopen("../semantic_traits.txt", U_WRITE));
	if (f == NULL)
		fatal_error("Unable to open file semantic_traits.txt for writing\n");

	u_set_trie::const_iterator i(d.semantic_traits.begin());
	++i;
	while (i != d.semantic_traits.end())
	{
		if (i->data)
		{
