...
#ifndef GRAPE_TO_FPRTN_PARSER_H
#define GRAPE_TO_FPRTN_PARSER_H
...
#include <grape/tag_input_traits.h>
#include <grape/rtno.h>
#include <grape/output_fprtn.h>

#ifdef TRACE
#include <grape/output_fprtn_to_dot_serializer.h>
#include <grape/inv_output_fprtn_to_dot_serializer.h>
#endif
...
namespace grape
{
	...
	struct to_fprtn_parser
	{
		...
		typedef ns_rtno<tag_input, tag_output> in_machine;
#else
		typedef rtno<tag_input, tag_output> in_machine;
#endif
		...
		typedef output_fprtn<SourceRef, TagOutput, TagInput, execution_state_set_impl_choice> out_machine;
...
#ifdef TRACE
					std::wcout << "added filtered pop transition from " << x.second.wlabel() << " to global acceptor state" << std::endl;
#endif
				}
			}
		};
...
#ifdef TRACE
								static_cast<paused_execution_state&>(*x_p_result.first).serialize(std::wcout) << " paused" << std::endl;
								static_cast<out_state&>(*x_t_result.first).serialize_earley_exe_state(std::wcout << "subanalysis already existent: ") << std::endl;
#endif
							}
						}
...
//			static_cast<out_state&>(*result.first).set_mapped(out.initial_state.second.src_ref);
		...
		void operator()(const in_machine& grammar, bool hasnt_white_at_begin, bool hasnt_white_at_end, out_machine& out)
		{
#ifdef TRACE
			std::wcout << "Begin parsing\n";
...
#ifdef TRACE
			std::wcout << "***** Paused ESs *****\n";
			std::wcout << "----- V[" << static_cast<int>(0) << "] -----\n";
